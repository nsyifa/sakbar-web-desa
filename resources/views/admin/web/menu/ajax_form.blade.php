<form action="{{ $form_action }}" method="post" id="validasi">
    <div class="modal-body">
        ...
        <div class="form-group" id="jenis_link" style="@if (!$menu['link_tipe']) display:none; @endif">
            ...
            <select id="halaman_baru" class="form-control input-sm jenis_link select2" name="{{ jecho($menu['link_tipe'], 1, 'link') }}" style="@if ($menu['link_tipe'] != 12) display:none; @endif">
                <option value="">-- Pilih Halaman Baru --</option>
                @foreach ($halaman_baru as $data)
                <option value="halaman/{{ $data['slug'] }}" @selected($menu['link']=="halaman/{$data['slug']}" )>{{ $data['judul'] }}</option>
                @endforeach
            </select>
